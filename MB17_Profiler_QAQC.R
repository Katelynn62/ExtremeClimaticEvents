###############################################################################
# QA/QC High Frequency Data - MB 2017
###############################################################################
library(tidyverse)
library(lubridate)
...
###############################################################################
str(Profiler_MB17)
Profiler_MB17 <- read_csv("Raw Data/Profiler/Missisquoi_2017_Profiler.csv") %>%
  ...
  mutate(ODOmgL = `ODO mg/l`) %>%
  mutate(ODOsat = `ODOsat`) %>%
  ...
  mutate(Remove = if_else(Depth == 0.5 & Turbid > 200, "Yes", "No"),
...
### Should be able to get away with using 2.5 for this analysis. 
...
write.csv(Profiler_MB17, file="MB_Profiler_2017_Clean.csv")
