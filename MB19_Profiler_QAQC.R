###############################################################################
# QA/QC High Frequency Data - MB 2019
###############################################################################
library(tidyverse)
library(lubridate)
...
###############################################################################
str(Profiler_SA18)
Profiler_MB19 <- read_csv("../Raw Data/Profiler/Missisquoi_2019_Profiler.csv") %>%
  ...
  mutate(Temp_C = Temp)%>%
  mutate(PC_RFU = `BGA PC RFU`) %>%
  mutate(Chl_RFU = `Chl RFU`) %>%
  mutate(Turbid = `Turbid+`) %>%
  mutate(pH = `pH...16`) %>%
  ...
  filter(Year == 2019) %>%
  mutate(Remove = if_else(PC_RFU > 100, "Yes", "No"),
...
write.csv(Profiler_MB19, file="..//Clean Data/MB_Profiler_2019_Clean.csv")
