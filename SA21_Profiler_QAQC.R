###############################################################################
# QA/QC High Frequency Data - SA 2021
###############################################################################
library(tidyverse)
library(lubridate)
...
###############################################################################
str(Profiler_SA21)
Profiler_SA21 <- read_csv("../Raw Data/Profiler/StAlbansInner_2021_Profiler.csv") %>%
  ...
  select(datetime, Month, Day, Year, Hour, Depth, Temp_C, ODOsat, ODOmgL, PC_RFU, Chl_RFU, pH, Turbid, SpCond) %>%
  mutate(PC_RFU = if_else(PC_RFU < 0, 0, PC_RFU)) %>%
  mutate(Chl_RFU = if_else(Chl_RFU < 0, 0, Chl_RFU)) %>%
  mutate(Remove = if_else(Depth == 1.5 & Month == 7 & Chl_RFU > 5, "Yes", "No"),
         Remove = if_else(Depth == 2 & Chl_RFU > 6, "Yes", Remove),
         Remove = if_else(Depth == 0.5 & Turbid > 20, "Yes", Remove),
         Remove = if_else(Depth == 1.5 & Month == 7 & Day < 15 & Turbid > 6, "Yes", Remove),
         Remove = if_else(Depth == 4.5 &  Turbid > 9, "Yes", Remove)) %>%
  filter(Remove == "No")
...
D05 %>%
  ...
  geom_point()
## Looks like there is a flat line of PC values in 2021. We should remove those in the analysis, 
################################################################################
...
write.csv(Profiler_SA21, file="..//Clean Data/SA_Profiler_2021_Clean.csv")
