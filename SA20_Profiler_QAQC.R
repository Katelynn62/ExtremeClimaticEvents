...
###############################################################################
str(Profiler_SA18)
Profiler_SA20 <- read_csv("../Raw Data/Profiler/StAlbansInner_2020_Profiler.csv") %>%
  ...
  mutate(Turbid= if_else(Turbid < 0, 0, Turbid)) %>%
  mutate(Remove = if_else(Depth == 0.5 & Chl_RFU > 8, "Yes", "No"),
         Remove = if_else(Depth == 1.0 & Chl_RFU > 7, "Yes", Remove),
         Remove = if_else(Depth == 1.5 & Month >= 10 & Chl_RFU > 5, "Yes", Remove),
         Remove = if_else(Depth == 2.5 & Month >=10 & Chl_RFU > 6, "Yes", Remove),
...
write.csv(Profiler_SA20, file="..//Clean Data/SA_Profiler_2020_Clean.csv")
