###############################################################################
# QA/QC High Frequency Data - SA 2018
###############################################################################
library(tidyverse)
library(lubridate)
...
###############################################################################
str(Profiler_SA18)
Profiler_SA19 <- read_csv("../Raw Data/Profiler/StAlbansInner_2019_Profiler.csv") %>%
  ...
  mutate(Depth = if_else(Depth >5.3 & Depth <=5.8, 5.5, Depth)) %>%
  mutate(Depth = if_else(Depth >5.8 & Depth <=6.3, 6.0, Depth)) %>%
  ...
  dplyr::select(datetime, Month, Day, Year, Hour, Depth, Temp_C, ODOsat, PC_RFU, Chl_RFU, pH, Turbid, SpCond) %>%
  mutate(PC_RFU = if_else(PC_RFU < 0, 0, PC_RFU)) %>%
  mutate(Chl_RFU = if_else(Chl_RFU < 0, 0, Chl_RFU)) %>%
  filter(Year == 2019) %>%
  mutate(Remove = if_else(Depth == 3.0 & Month == 6 & Day > 15 & PC_RFU > 2, "Yes", "No"),
         Remove = if_else(Depth == 4.0 & Month == 6 & PC_RFU > 5, "Yes", Remove),
         Remove = if_else(Depth == 0.5 & Chl_RFU > 17, "Yes", Remove),
         Remove = if_else(Depth == 1.5 & Chl_RFU > 17, "Yes", Remove),
         Remove = if_else(Depth == 2.0 & Chl_RFU > 18, "Yes", Remove),
         Remove = if_else(Depth == 2.5 & Month >= 10 & Chl_RFU > 13, "Yes", Remove), 
         Remove = if_else(Depth == 3.0 & Chl_RFU > 20, "Yes", Remove), 
         Remove = if_else(Depth == 3.0 & Month >= 10 & Chl_RFU > 15, "Yes", Remove),
         Remove = if_else(pH < 2, "Yes", Remove),
         Remove = if_else(Depth == 0.5 & Turbid > 50, "Yes", Remove)) %>%
  filter(Remove == "No")
...
D05 <-  Profiler_SA19 %>%
  dplyr::select(PC_RFU, datetime, Depth) %>%
  filter(Depth == "0.5") %>%
...
D05 %>%
  ...
  geom_point()
## Looks like there is a flat line of PC values in 2018. We should remove those in the analysis, 
################################################################################
...
D05 <-  Profiler_SA19 %>%
  dplyr::select(Chl_RFU, datetime, Depth) %>%
  filter(Depth == "0.5") %>%
...
D05 <-  Profiler_SA19 %>%
  dplyr::select(Temp_C, datetime, Depth) %>%
  filter(Depth == "0.5") %>%
...
D05 <-   Profiler_SA19 %>%
  dplyr::select(ODOsat, datetime, Depth) %>%
  filter(Depth == "0.5") %>%
...
D05 <-  Profiler_SA19 %>%
  dplyr::select(pH, datetime, Depth) %>%
  filter(Depth == "0.5") %>%
  mutate(Lag = lag(pH)) %>%
...
D05 <- Profiler_SA19 %>%
  dplyr::select(Turbid, datetime, Depth) %>%
  filter(Depth == "0.5") %>%
...
write.csv(Profiler_SA19, file="..//Clean Data/SA_Profiler_2019_Clean.csv")
